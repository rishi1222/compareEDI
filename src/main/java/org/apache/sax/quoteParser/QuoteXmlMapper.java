 ...
 */
public class QuoteXmlMapper extends Mapper<LongWritable, Text, Text, MapWritable> {
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if(attributes.getLength() == 1)
                    {
                        ...
                        if(node.getNodeName().equalsIgnoreCase("QuoteId"))
                        {
                            quoteId = node.getTextContent();
                        ...
                        src.put(new Text(nodeName.getBytes("UTF-8")), new Text(quoteId.getBytes("UTF-8")));
